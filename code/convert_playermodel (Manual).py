...
def createProportionTrickAction(armature):
	bpy.ops.object.mode_set(mode="OBJECT")
	...
	action = bpy.data.actions.new(name="proportion_trick")
	...
	for track in armature.animation_data.nla_tracks:
		if track.name == "proportion_trick":
			armature.animation_data.nla_tracks.remove(track)

	track = armature.animation_data.nla_tracks.new()
	track.name = "proportion_trick"
	strip = track.strips.new("proportion_trick", 0, action)
	strip.frame_end = 1
...
# 10. Preserve original mesh weights
#
# The original script automatically weight painted metacarpals here and then
# limited every mesh to 4 vertex groups. For this conversion pass, keep the
# imported model's weights untouched so twist and shoulder weights can be tested.
...
createProportionTrickAction(arm_b)
